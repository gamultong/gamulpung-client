...
export default function useMovement({
  ...
  const moveCursor = (
    ...
    const moveAnimation = (dx: number, dy: number) => {
      ...
      const shockwave = document.getElementById('ShockwaveCanvas');
      const bombMarker = document.getElementById('BombMarkerCanvas');
      const currentRefs = [I_canvas.current, C_canvas.current, P_canvas.current, tilemap, colorOverlay, shockwave, bombMarker];
